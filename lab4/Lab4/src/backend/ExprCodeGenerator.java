...
public class ExprCodeGenerator extends Visitor<Value> {
	...
	public Value visitStringLiteral(StringLiteral nd) {
		/* TODO: return something meaningful here */
		Value string = StringConstant.v(nd.getValue());
		return string;
	}
	...
	public Value visitBooleanLiteral(BooleanLiteral nd) {
		...
		//take string value and convert to 0 or 1?
		Boolean v = nd.getValue();
		if(v == true){
	...
	public Value visitBinaryExpr(BinaryExpr nd) {
		 ...
		 */
		//get values for left and right side of expression
		final Value left = wrap(nd.getLeft().accept(this));
		final Value right = wrap(nd.getRight().accept(this));

		//use anonymous class for adding methods(add, sub, mul, div and rem)
		Value res = nd.accept(new Visitor<Value>() {
			...
			@Override
			//method uses mod instead of remainder since the sign is modulo
